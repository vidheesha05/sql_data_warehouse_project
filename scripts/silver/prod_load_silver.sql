/* 
Stored Procedure: Bronze to Silver

This Stored Procedure performs ETL (Extract, Transform, Load) process to populate
the silver layer schema tables from the bronze layer schema tables.
*/
...
BEGIN
	...
	BEGIN TRY
		...
		PRINT'=======================================';
		PRINT'DATA TRANSFORMATIONS IN CRM TABLES';
		PRINT'=======================================';

        --Data Cleaning of crm_cust_info table in silver layer
		...
		SELECT 
			...
			TRIM(cst_lastname) as cst_lastname,   -- Removed Unwated Spaces
			CASE 
				 WHEN UPPER(TRIM(cst_marital_status)) = 'M' THEN 'Married' -- Data Standardisation & Consiste
		...
		--Data Cleaning of crm_prd_info table in silver layer
		SET @start_time = GETDATE();
		...
		SELECT
			...
			CASE 
				 ...
				 ELSE 'n/a'                      -- Map product line codes to descriptive values and handling null values
			END AS prd_line,
			CAST(prd_start_date AS DATE) AS prd_start_date,      -- Changing the datatype (data typecasting)
			CAST(
		...
		--Data Cleaning of CRM sales details table in silver layer
		SET @start_time = GETDATE();
		...
		--Data Cleaning of ERP Customer Information table
		SET @start_time = GETDATE();
		...
		--Data Cleaning of ERP Customer Location table
		SET @start_time = GETDATE();
		...
		--Data Cleaning for ERP Product Table
		SET @start_time = GETDATE();
